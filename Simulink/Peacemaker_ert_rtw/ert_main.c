...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Peacemaker_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Peacemaker_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Peacemaker_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Peacemaker_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Peacemaker_M, 0);

  /* Initialize model */
  Peacemaker_initialize();
