...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Averynicefile_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Averynicefile_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Averynicefile_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Averynicefile_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Averynicefile_M, 0);

  /* Initialize model */
  Averynicefile_initialize();
