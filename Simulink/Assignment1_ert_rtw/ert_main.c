...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Assignment1_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Assignment1_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Assignment1_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Assignment1_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Assignment1_M, 0);

  /* Initialize model */
  Assignment1_initialize();
