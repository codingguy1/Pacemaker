...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Assignment1and2_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Assignment1and2_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Assignment1and2_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Assignment1and2_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Assignment1and2_M, 0);

  /* Initialize model */
  Assignment1and2_initialize();
